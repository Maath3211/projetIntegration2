                ...
                <li class="result-item" style="padding: 10px; border-bottom: 1px solid #ccc; display: flex; align-items: center; justify-content: space-between;">
                    <div class="profile-trigger" style="display: flex; align-items: center;">
                        @if($demande->requester_avatar)
                            <img src="{{ asset('images/avatars/' . $demande->requester_avatar) }}" alt="{{ __('friends.profile_avatar') }}" class="avatar" style="width:40px;height:40px;border-radius:50%; margin-right:10px;">
                        @else
                            <img src="{{ asset('images/avatars/default-avatar.jpg') }}" alt="{{ __('friends.default_avatar') }}" class="avatar" style="width:40px;height:40px;border-radius:50%; margin-right:10px;">
                        @endif
                    ...
                    <div>
                        ...
                        <form action="{{ route('amis.accepter') }}" method="POST" style="display:inline-block;">
                            ...
                            <button type="submit" class="add-button" style="background-color: #a9fe77; border: none; padding: 8px 12px; border-radius: 5px; cursor: pointer;" title="{{ __('friends.accept') }}">{{ __('friends.accept') }}</button>
                        </form>
                        <!-- Bouton refuser -->
                        <form action="{{ route('amis.refuser') }}" method="POST" style="display:inline-block; margin-left:5px;">
                            @csrf
                            <input type="hidden" name="demande_id" value="{{ $demande->id }}">
                            <button type="submit" class="cancel-button" style="background-color: #ff7b7b; border: none; padding: 8px 12px; border-radius: 5px; cursor: pointer;" title="{{ __('friends.decline') }}">{{ __('friends.decline') }}</button>
                        </form>
                    </div>
                </li>
            @endforeach
